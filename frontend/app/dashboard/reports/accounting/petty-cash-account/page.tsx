...
export default function PettyCashAccountReportPage() {
  ...
  useEffect(() => {
    ...
    const loadReport = async () => {
      try {
        setLoading(true);
        setErrorMessage('');
        const pettyRows = await fetchAll<PettyCashRow>('/petty-cash-transactions');
        setRows(pettyRows);
      } catch (error) {
        console.error('Failed to load petty cash report data:', error);
        setErrorMessage('Unable to load petty cash report data. Please try again.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const rowToExport = (row: PettyCashRow) => [
    String(row.date || '-'),
  ...
  const exportCsv = () => {
    ...
    link.href = url;
    link.download = `petty-cash-account-report-${new Date().toISOString().split('T')[0]}.csv`;
  ...
  const downloadPdf = () => {
    ...
    doc.setFontSize(16);
    doc.text('Petty Cash Account Report', 40, 40);
    ...
    doc.save(`petty-cash-account-report-${new Date().toISOString().split('T')[0]}.pdf`);
  };
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-emerald-50 via-teal-50 to-cyan-50">
        <div className="rounded-2xl border border-emerald-100 bg-white/85 px-6 py-5 shadow-xl backdrop-blur">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-slate-800">Preparing Petty Cash Report</p>
              <p className="text-xs text-slate-500">Loading petty cash transactions...</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <main className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8 space-y-6 relative z-10">
        <section className="rounded-3xl border border-white/70 bg-white/80 backdrop-blur-lg p-6 md:p-7 shadow-2xl">
          <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
            <div>
              <p className="inline-flex items-center rounded-full bg-slate-900 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.12em] text-white">Petty Cash Account Report</p>
              <h1 className="mt-3 text-3xl md:text-4xl font-black tracking-tight text-slate-900">Petty Cash Inflow & Outflow</h1>
              <p className="mt-2 text-sm md:text-base text-slate-600 max-w-2xl">
                Analyze petty cash top-ups and expenses with date filters, search controls, and downloadable report outputs.
              </p>
            </div>
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <article className="rounded-2xl border border-emerald-200 bg-gradient-to-br from-emerald-50 to-white p-5 shadow-lg">
            ...
            <p className="mt-2 text-xs text-emerald-700">Top-ups and incoming entries</p>
          </article>
          <article className="rounded-2xl border border-rose-200 bg-gradient-to-br from-rose-50 to-white p-5 shadow-lg">
            ...
            <p className="mt-2 text-xs text-rose-700">Expense and outgoing entries</p>
          </article>
        ...
        <section className="rounded-2xl border border-white/70 bg-white/90 shadow-xl overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100 bg-gradient-to-r from-slate-50 to-emerald-50">
            <h2 className="font-semibold text-slate-900">Petty Cash Transactions</h2>
